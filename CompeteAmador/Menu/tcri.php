...
$dataAtual = date('Y-m-d'); // Certifique-se de que $dataAtual esteja corretamente formatada
// Consulta para exibir os torneios criados pelo usuário, incluindo categoria, gênero e detalhes do torneio
   $query = "
    SELECT 
        t.cod, 
        t.idcri, 
        ...
        m.id AS idmoda,  -- Aqui estamos selecionando o id da tabela 'modalidades'
        t.njogadores, 
        u.uniformes AS uniforme, 
        t.bairro, 
        t.rua, 
        t.responsavel, 
        t.telefonet
    ...
    LEFT JOIN 
        modalidades m ON t.modalidades = m.id  -- Aqui
    LEFT JOIN 
        uniformes u ON t.uniformes = u.id
    WHERE 
        t.idcri = $usuario_id
        AND 
        t.datainicio >= '$dataAtual'
";
...
<head>
    ...
    <title>Competições criadas</title>
    <!-- Google Fonts -->
...
<body>
 ...
 <form action="../logout.php" method="post">
  ...
  <h1 class="text-center mt-2" style="color: white;">Lista de Competições Criados</h1>
...
<?php if (mysqli_num_rows($seleciona) > 0): ?>
    <?php while ($campo = mysqli_fetch_array($seleciona)): ?>
        ...
        <div class="card mt-4">
            ...
            // Supondo que o valor da modalidade vem do banco (exemplo: $campo['modalidades'])
            $idmoda = $campo['modalidades']; // Aqui pega o ID da modalidade
            ...
            <div class="card-footer">
                <button 
                    class="btn btn-info" 
                    data-bs-toggle="modal" 
                    data-bs-target="#modalDescricao<?= $campo['cod'] ?>" 
                    style="font-size: 14px; padding: 5px 10px; border-radius: 5px; margin-right: 10px;">
                    Descrição
                </button>
                <button 
                    class="btn btn-success" 
                    data-bs-toggle="modal" 
                    data-bs-target="#modalCompradores<?= $campo['cod'] ?>" 
                    style="font-size: 14px; padding: 5px 10px; border-radius: 5px;">
                    Ver quem comprou
                </button>
            </div>
        </div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <p><strong>Categoria e Gênero:</strong> <?= $campo['categoriaNome'] ?> <?= $campo['generoNome'] ?></p>
                            <p><strong>Máximo de Atletas:</strong> <?= $campo['njogadores'] ?></p>
                            <p><strong>Uniforme:</strong> <?= $campo['uniforme'] ?></p>
                            <p><strong>Estado:</strong> <?= $estado['nome'] ?> - <?= $estadoUF ?></p>
                            <p><strong>Cidade:</strong> <?= $cidadeNome ?></p>
                            <p><strong>Bairro:</strong> <?= $campo['bairro'] ?></p>
                            <p><strong>Rua:</strong> <?= $campo['rua'] ?></p>
                        ...
                        <!-- Modal para Compradores -->
                        <div class="modal fade" id="modalCompradores<?= $campo['cod'] ?>" tabindex="-1" aria-labelledby="modalLabelCompradores<?= $campo['cod'] ?>" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="modalLabelCompradores<?= $campo['cod'] ?>">Compradores das Competições: <?= $campo['nomet'] ?></h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <p><strong>Nome da Competição:</strong> <?= $campo['nomet'] ?></p>
                            <!-- Aqui você pode adicionar a lógica para exibir a lista de compradores, se disponível -->
                            <p>Listagem dos compradores irá aqui.</p>
                            <table class="table">
    <thead>
        <tr>
            <th class="data-separador">Nome do Comprador</th>
            <th class="data-separador">Vagas Compradas</th>
            <th>Data da Compra</th>
        </tr>
    </thead>
    <tbody>
        <?php
        // Consulta para obter os compradores do torneio com os nomes
        $queryCompradores = "
        SELECT u.nome, c.vagascompradas, c.datacom
        FROM compra c
        JOIN usuario u ON c.id_usuario = u.cod
        WHERE c.id_torneio = {$campo['cod']}
        ";    
        $resultCompradores = mysqli_query($conexao, $queryCompradores);
    
        if (mysqli_num_rows($resultCompradores) > 0):
            while ($comprador = mysqli_fetch_array($resultCompradores)):
                // Formata a data no formato desejado
                $dataFormatada = date("H:i d/m/Y", strtotime($comprador['datacom']));
                ?>
                <tr>
                    <td class="data-separador"><?= htmlspecialchars($comprador['nome']) ?></td>
                    <td class="data-separador"><?= htmlspecialchars($comprador['vagascompradas']) ?></td>
                    <td><?= htmlspecialchars($dataFormatada) ?></td>
                </tr>
            <?php endwhile; 
        else: ?>
            <tr>
                <td colspan="3">Nenhum comprador registrado para este torneio.</td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
    ...
    <?php endwhile; ?>
<?php else: ?>
    <p class="text-center" style="color: black; font-size: 2em; font-weight: bold; margin-top: -1em;">Nenhum competição criada</p>
<?php endif; ?>
